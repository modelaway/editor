...
export default class Views {
  ...
  async add( name: string, to: string, triggerType?: AddViewTriggerType ){
    const component = await this.flux.getComponent( name )
    if( !component )
  ...
  async lookup( e: Event ){
    ...
    let component = await this.flux.getComponent( cname, $currentTarget )
   ...
   * Target: Native HTML tags or custom views
   */
  async propagate( $node: JQuery<HTMLElement> ){
    ...
    const component = await this.flux.getComponent( cname )
    if( component?.name ){
      /**
       * Component's name can be the same as its HTML 
       * nodeName identifier.
